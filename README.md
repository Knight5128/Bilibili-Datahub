...
这是一个面向本地研究与分析场景的 Bilibili 数据工具仓库，当前主要包含两个本地 Streamlit 应用、一个运行在 `Cloud Run` 上的追踪服务，以及一个运行在 `Cloudflare Workers` 上的公网控制台：
...
- `src/`：核心业务逻辑与数据处理模块，现已包含 `bili_pipeline.cloud_tracker` 云端追踪服务。
- `tests/`：基础测试用例。
- `assets/`：界面资源文件。
